...
splitted_input.each_with_index do |line, index|
  next if [0,1].include?(index)
  splitted_line = line.gsub("=", "")
                      .gsub("(", "")
                      .gsub(")", "")
                      .gsub(",", "")
                      .split(" ")
...
counter = 0
current_element = steps.find{ |step| step[:element] == "AAA" }

while current_element[:element] != "ZZZ" do
  current_instruction_index = counter % instruction_sequence.count
  current_instruction = instruction_sequence[current_instruction_index]
  next_element = current_element[current_instruction.to_sym]
  current_element = steps.find{ |step| step[:element] == next_element }
  counter += 1
end
